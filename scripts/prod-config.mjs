 ...
 *
 * A deploy is often mid-flight (or queued) when CI / a local run calls `ampx
 * generate outputs` — which hard-fails with DeploymentInProgressError. That's a
 * transient race, not a real failure.
 *
 * Strategy (hybrid):
 *   1. Poll `aws amplify list-jobs` until no RUNNING/PENDING deploy — logging
 *      what we're waiting on. Best-effort: if creds can't call ListJobs (or the
 *      CLI is absent), we skip straight to step 2.
 ...
 *
 * NOTE: APP_ID is a placeholder until the `main` branch backend is created in
 * Amplify Hosting. Set it to the real app id (and confirm the region) then.
 */
